...
Before we start our server to see if this works, let’s update the server configuration to expose the API on `/v1/` rather than the default path (`/api/`)
