...
I was nervous, excited, anxious, and more to be presenting at NodeSummit, this would be the largest public presentation. I had been practicing for days with co-workers, improving the talk and slides based on their feedback. I was ready, 10 minutes till my talk and my biggest fear was a tech malfunction (laptop, projector, mic, etc.) despite the NodeSummit team being ready for everything. I got on stage to an almost packed room, took a deep breath, clock struck 4:20 and off I went, starting to deliver my talk. The audience was enthusiatic and engaged and the nervousness disappeared within seconds of starting (maybe minutes). After the talk I had a lot of people approach me to say they enjoyed the talk (phew) while others had questions about some of the content. People were sharing how they faced the same pain points and the talk was super relatable. It was very exciting to answer the questions and receive feedback and comments from the attendees. Overall, I was thrilled to present NodeSummit and hope to be able to do so again next year. 
...
There were some followup question I got from more than one person that I would like to answer here for everyone: 

**So the `@loopback/context` module can be used standalone?**

YES! The Context module is designed to work wihtout any other LoopBack dependency and can serve as a Universal Container for your application / framework.
...
Check out the `@loopback/context` packaage [here](hhttps://www.npmjs.com/package/@loopback/context).

**What's the best way to transition to TypeScript in an existing project?**

TypeScript compiles to JavaScript! You can start to switch incrementally by writing new modules in you project using TypeScript and it should all compile without issues. This way you get experience with TypeScript and when you have time, older modules can be transitioned to TypeScript at a later date. You might even benefit from the TypeScript compiler catching simple errors in JavaScript if you run the code through it.

For those looking to setup a TypeScript project, check out the [`@loopback/build`](https://www.npmjs.com/package/@loopback/build) package to simplify it as it provides defaults for `typescript` builds, `tslint`, `prettier`, `mocha`, etc.

**When is LoopBack 4 coming out?**

Its already out! LoopBack 4 is available as a Developer Preview #3 and can be used to start building applications. We are adding more features and fixing bugs based on the community's feedback. If all goes according to plan, LoopBack 4 GA (General Availability) for building production application should be sometime in October 2018. That said, I would recommend getting started now as I only anticipate new features and limited (if any) breaking changes to the developer preview. 

**Can you tell me more about Call for Code?**
IBM is a founding partner for Call for Code, an initiative by David Clark Cause. [Call for Code](https://callforcode.org/) is a multi-year global initiative is a rallying cry to developers to use their skills and mastery of the latest technologies, and to create new ones, to drive positive and long-lasting change across the world with their code. The inaugural Call for Code Challenge theme is Natural Disaster Preparedness and Relief. Grand Prize is $200,000 USD!! There's still time to answer the call.

## Training Days

Its important for me to always keep learning and NodeSummit presented a great opportunity by offering their first ever Training Days! The two training days following the conference were run by experts and offered a wide variety of topics. These were half-day sessions held in small groups so you could easily interact with the instructor. I attended 4 sessions and I learned something from each one. In particular, the sessions I attended were:
